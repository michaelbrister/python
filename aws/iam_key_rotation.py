...
def createAccessKey(session, oldCredentials):
    ...
    try:
        response = iam.create_access_key(UserName=oldCredentials["UserName"])
...
def getProfiles(aws_credentials_file):
    ...
    return [
        profile for profile in config.sections() if config[profile]["aws_access_key_id"]
    ]
...
def updateCredentialsFile(profile, newCredentials, aws_credentials_file):
    ...
    config.set(
        profile, "aws_secret_access_key", newCredentials["AccessKey"]["SecretAccessKey"]
    )
...
@backoff.on_exception(backoff.expo, ClientError, on_backoff=backoff_hdlr, max_time=30)
def deleteAccessKey(session, iamCredentials):
    ...
    """
    logging.info(
        f"Using session with access key {session.get_credentials().access_key}"
    )
    ...
    iam.delete_access_key(
        AccessKeyId=iamCredentials["AccessKeyId"], UserName=iamCredentials["UserName"]
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Rotates AWS IAM Keys for specified profile(s)"
    )
    parser.add_argument(
        "-p",
        "--profiles",
        type=str,
        help="List of comma seperated AWS Profiles to update",
    )
    parser.add_argument(
        "-f",
        "--force",
        default=False,
        action="store_true",
        help="When there are two access keys, deletes the access key the current request is not using. ",
    )
    parser.add_argument(
        "--safe",
        default=False,
        action="store_true",
        help="Backs up the current config. And only deactivates the old key.",
    )
    parser.add_argument(
        "--apply",
        default=False,
        action="store_true",
        help="Performs the key rotation instead of just doing a check.",
    )
    ...
    for profile in PROFILES:
        ...
        if len(iamKeys["AccessKeyMetadata"]) > 1 and args.force is not False:
            logging.info(
                f"Multiple access keys found and force flag is not set, unable to rotate keys for profile {profile}."
            )
            exit()

        if len(iamKeys["AccessKeyMetadata"]) > 1 and args.force:
            logging.info(
                "Multiple Access keys found, force flag is set. Deleting unused access key."
            )
            ...
            logging.info(
                next(
                    (
                        key["AccessKeyId"]
                        for key in iamKeys["AccessKeyMetadata"]
                        if key["AccessKeyId"] != session.get_credentials().access_key
                    ),
                    None,
                )
            )
            ...
            oldCredentials["AccessKeyId"] = next(
                (
                    key["AccessKeyId"]
                    for key in iamKeys["AccessKeyMetadata"]
                    if key["AccessKeyId"] != session.get_credentials().access_key
            ...
            logging.info(
                "AWS IAM APIs employ an eventually consistent model\nSleeping to allow AWS time to propigate the keys."
            )
            time.sleep(5)
        ...
        new_session = boto3.Session(
            aws_access_key_id=newCredentials["AccessKey"]["AccessKeyId"],
